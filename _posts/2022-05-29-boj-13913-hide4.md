...
이번 문제 또한 Node를 이용해 해결하였다.

각 지점별로 경로를 저장하면 시간초과가 자꾸 발생하였다. (String을 더하면서 해결하려 했었다.)
     -> 알아보니 자바 String 더하기의 경우 O(N+K)의 시간복잡도를 가진다. 그런데 while(!q.isEmpty)로 O(N)번 돌리면 최대 시간 복잡도는 O(N^2)으로 최대 10000000000*K 번 돌 수 있으므로
         시간초과가 날 수 있다.... 
